...
这不是旧版本的滚动升级指南。0.3 使用协议 v2，Server 不再有业务数据端口，也不允许旧数据面与 0.3 混跑。
...
Home 的真实部署仍需在被版本控制忽略的私有目录中复制样例为 `home-bootstrap.toml`，并通过私密渠道放入 `deployment-root.pub` 和 `deployment-trust.json`。这些私有材料只能在目标机器或受控私有分发流程中组合，不得进入 Git、构建日志、CI 日志、Issue、PR、公开 Release、截图、公共 Wiki 或聊天输出。Travel 首次注册不需要这些文件，只需人工输入一个可达 Relay 地址。
...
Travel 机器不预先创建运行期 TOML 或 cert；从公开包目录执行：
