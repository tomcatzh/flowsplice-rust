...
| M1 — Home session can be silently replaced and is not pinned by Server | Confirmed. | Server now requires every accepted `home_id` to occur in the verified deployment trust and pins it to that root-bound management SPKI key set. A valid replacement explicitly closes and logs only the old session for that same Home ID; other Home sessions remain isolated. Relay applies the same explicit supersession discipline to Server sessions. |
