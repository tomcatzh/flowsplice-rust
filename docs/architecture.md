...
```text
Home Agent A --outbound mTLS--┐       ┌---> passive Relay (Server initiates) <---mTLS---┐
                              ├--> Server                                               Travel Agent
Home Agent B --outbound mTLS--┘       └<--- active Relay (Relay initiates)  <---mTLS---┘
```

Each Home publishes its own catalog. Server keeps an explicit configured set of permitted Home IDs, derives each Home's management SPKI key set from the verified deployment trust, keeps one replaceable session per Home ID, and builds a sorted aggregate catalog. Reconnecting one Home supersedes only that ID's old session; disconnecting it removes only its catalog. Every Relay has an explicit connection mode. For a passive Relay, Server dials one stable management seed; for an active Relay, Relay dials Server and Server stores no Relay address. After mutual identity checks, either direction becomes the same long-lived bidirectional control session. Relay reports or withdraws its current management/data listener endpoints on that session, and Server binds the directory SPKI to the authenticated Relay certificate. Server rebuilds one sorted, generation-numbered in-memory directory after every add, update, withdrawal, or disconnect and broadcasts the complete replacement snapshot to every connected Relay; each Relay atomically replaces its local in-memory copy and rejects same-generation divergence. Directory changes and signed Travel snapshots allocate from the same persisted control-generation high-water mark, so a Server restart cannot roll the directory generation backward. A seed is reachability input only and is never published as endpoint truth. Address updates do not replace the control session. Each Relay ID has one replaceable authenticated session, and a connection direction inconsistent with its configured mode is rejected.
...
Server binds exactly one explicit Home-control/active-Relay address shared by configured identities and one loopback-only statistics UI. It also originates control connections to passive Relays. It has no business-data listener and never pairs or forwards business sockets. Each Relay owns its data listener and directly pairs the exact Travel and Home sockets authorized for a route.
...
A normal Home authority is root-certified only for its own Home and cannot mint Global or another Home's grant. Home decrypts the signing keys only for the request, verifies the CA and authority material against the signed deployment trust, signs the two CSR public keys under separate CA roots, then signs one credential that atomically binds the deployment/trust, request ID, independent 256-bit nonce and full request hash, both SPKIs, both exact CA and leaf-certificate hashes, authority epoch, credential ID, scope, and validity. It returns one self-contained Enrollment Response, zeroizes the password, and publishes only the signed public grant to Server. The waiting `enroll-remote` process verifies the root-signed trust discovered from the operator-selected Relay, reconnects with the verified Management CA, and then verifies every response binding, encrypted local key, certificate chain, and grant.
...
A fresh Travel uses a deployment-neutral binary. The operator enters one reachable Relay management
address; no deployment file is placed in the package or prepared on the device. Travel retrieves the
deployment root public key and signed trust through a discovery-only exchange, verifies the signature,
reconnects with the verified Management CA, generates encrypted keys, a
...
On OpenWrt, one package installs the Server and Relay executables, a UCI-to-TOML renderer, one procd service, and one LuCI page with standard Chinese/English catalogs. Server exposes one LAN-only Home control/active-Relay listener, a loopback statistics listener, a mode-aware UCI list of Relay identities, and paths for the deployment-root public key, signed deployment trust, Server control-signing key, and redb state. The local LAN and WAN6 Relays use active mode and connect to that stable LAN Server address; Server stores neither local Relay address. Each Relay section has independent management/data listeners, one required OpenWrt logical network, authorization cache, redb state, and loopback statistics listener. Exact listener addresses are reported directly without a duplicate advertised-address field. Wildcard listeners read `l3_device`, `ipv4-address`, and `ipv6-address` from netifd over local ubus, bind the resolved L3 device, select the matching address family, and update or withdraw the endpoint over the existing control session as interface state changes. Authority records come from signed trust and are not duplicated in UCI. procd runs one named Server instance and one named process per enabled Relay section as the `flowsplice` account, and validation runs with that same account after rendered files are assigned to it. Separate LAN, WAN IPv4, and WAN IPv6 Relay identities therefore share binaries and administration without sharing listener configuration or lifecycle state. LuCI configures and controls Server/Relay only; issuance and revocation remain on Home. The package is inert by default and leaves firewall policy outside its authority.
