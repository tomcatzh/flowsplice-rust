...
env "${linker_name}=${toolchain}/${rust_target}34-clang" "${cc_name}=${toolchain}/${rust_target}34-clang" "${ar_name}=${toolchain}/llvm-ar" cargo build --locked --profile android-release --target "$rust_target" -p flowsplice-pty-native --manifest-path "$repo_root/Cargo.toml"
