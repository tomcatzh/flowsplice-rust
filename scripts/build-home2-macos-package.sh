...
sample_config_file="${repo_root}/homeagent/bootstrap.example.toml"
...
if [[ ! -f "${sample_config_file}" ]]; then
  printf 'Missing public Home bootstrap sample.\n' >&2
  exit 1
fi
...
cp "${sample_config_file}" "${package_root}/home-bootstrap.example.toml"
...
(
  ...
  shasum -a 256 \
    ...
    home-bootstrap.example.toml > SHA256SUMS
)
