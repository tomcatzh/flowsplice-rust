...
(cd "${repo_root}" && cargo build --locked --release \
  ...
  -p flowsplice-issuer -p flowsplice-foobar)
for binary in flowsplice-server flowsplice-relay flowsplice-homeagent flowsplice-travelagent flowsplice-issuer flowsplice-foobar; do
