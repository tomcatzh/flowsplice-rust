...
RUN test "$RUST_MIRROR_URL" = off && apk add --no-cache clang cmake make musl-dev perl python3
WORKDIR /src
...
RUN --mount=type=cache,id=flowsplice-pty-tests-registry-${TARGETARCH},target=/usr/local/cargo/registry,sharing=locked \
    ...
    cargo test --locked --no-run --message-format=json -p flowsplice-pty-home -p flowsplice-pty-client -p flowsplice-pty-protocol -p flowsplice-transport > /tmp/test-artifacts.json && \
    mkdir /tests && \
