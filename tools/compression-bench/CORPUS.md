...
| `repo-rust-16k`, `repo-rust-96k` | Genuine `.rs` files under `crates/`, `internal/`, `pty/`, `travel-android/rust/` and `travel-apple/rust/`, sorted by path, concatenated once, then naturally chunked at 16 / 96 KiB. No file repetition or padding. |
...
The markdown allowlist is `README.md`, `docs/architecture.md`, `docs/cryptography.md`, `docs/socket-runtime.md`, `docs/pty.md`, `pty-apple/README.md`, `pty-android/README.md`, and `openwrt/README.md`. No broad documentation/home-directory scan is performed. Rust discovery only visits `crates/`, `internal/`, `pty/`, `travel-android/rust/` and `travel-apple/rust/`, and does not follow symlinks. Actual source paths are recorded in each dataset's provenance. Each concatenated repository corpus and PNG is bounded at 16 MiB; missing allowlisted inputs fail rather than silently substituting synthetic bytes. Final repository chunks retain their natural short size. In particular, the 96 KiB documentation corpus may have fewer than eight blocks: the real documents are not repeated to manufacture sample count.
