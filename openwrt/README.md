...
- one Chinese/English LuCI page for configuration, validation, lifecycle controls, instance status, and the public half of offline Travel issuance;
...
Travel access is distributed as offline-signed credentials. The LuCI page can upload a Travel enrollment request, download the bounded Server approval, and later upload the signed response for live import. The offline signing machine and its management CA, business CA, and Travel-authorization private keys never belong on OpenWrt. Only the corresponding public trust material is installed there. The default validity is 365 days and is configurable before preparing the approval.

The Server keeps an add-only authorization set and revocation log, and each Relay keeps a persistent anti-rollback cache below `/etc/flowsplice/state`, allowing import or revocation to take effect without restarting any process. Validate before starting:
