...
async fn handle_bootstrap_travel(
    ...
    if let ControlMessage::BootstrapDiscoveryRequest { protocol_version } = message {
        ...
        let relay_data_addr = state
            .advertised_endpoint
            .borrow()
            .as_ref()
            .context("Relay endpoint is not currently available for discovery")?
            .data_public_addr
            .clone();
        #[cfg(feature = "e2e-discovery-address")]
        let relay_data_addr = std::env::var("FLOWSPLICE_E2E_DISCOVERY_DATA_ADDR")
            .ok()
            .filter(|address| !address.trim().is_empty())
            .unwrap_or(relay_data_addr);
        write_json(
            &mut stream,
